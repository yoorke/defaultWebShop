...
function AddToWishList() {
    ...
    $.ajax({
        ...
        success: function success(msg) {
            ...
            if (msg.d == 'Not loggedin') window.location = '/prijava?returnUrl=' + window.location.pathname;else $('#wishListMessageBox').show();
        },
...
function GetComparePageUrl(productList, blank) {
    ...
    if (blank) var win = window.open(url.substring(0, url.length - 1), '_blank');else var win = window.open(url.substring(0, url.length - 1), '_self');
}
...
function ShowHideButtons(divButtons, show) {
    if (show) $('#' + divButtons).show();else $('#' + divButtons).hide();
}
